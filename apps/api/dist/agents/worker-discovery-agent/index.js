...
exports.WorkerDiscoveryAgent = void 0;
const sqlite_1 = require("../../db/sqlite");
const uuid_1 = require("uuid");
class WorkerDiscoveryAgent {
    static async getOpportunities(workerId) {
        const startTime = Date.now();
        const runId = (0, uuid_1.v4)();
        console.log(`[WorkerDiscoveryAgent] Scanning opportunities for worker profile: ${workerId}`);
        try {
            // 1. Fetch worker profile
            let wp = sqlite_1.db.prepare('SELECT * FROM worker_profiles WHERE id = ?').get(workerId);
            if (!wp) {
                wp = sqlite_1.db.prepare('SELECT * FROM worker_profiles WHERE user_id = ?').get(workerId);
            }
            if (!wp) {
                throw new Error(`Worker profile not found for ID: ${workerId}`);
            }
            const workerSkills = JSON.parse(wp.skills);
            ...
            const contractorReqs = sqlite_1.db.prepare(`
      ...
      `).all(now);
            ...
            const customerReqs = sqlite_1.db.prepare(`
      ...
      `).all(now);
            ...
            for (const req of contractorReqs) {
                ...
                if (dist > maxRadius + 10)
                    continue; // Skip if too far (allow a little buffer)
            ...
            for (const sr of customerReqs) {
                const srSkills = JSON.parse(sr.extracted_skills);
      ...
      `).run(runId, JSON.stringify({ workerId }), JSON.stringify({ opportunitiesCount: opportunities.length }), latency, new Date().toISOString());
...
exports.WorkerDiscoveryAgent = WorkerDiscoveryAgent;
exports.default = WorkerDiscoveryAgent;
