...
exports.VoiceInteractionAgent = void 0;
const sqlite_1 = require("../../db/sqlite");
const uuid_1 = require("uuid");
class VoiceInteractionAgent {
    static async run(commandId, agentRunId) {
        ...
        try {
            ...
            const command = sqlite_1.db.prepare(`
      ...
      `).get(commandId);
            ...
            const lang = command.detected_language;
            let intent = 'UNKNOWN';
            let skill = undefined;
            ...
            else if (textLower.includes('registration') ||
                textLower.includes('register') ||
                textLower.includes('naya account')) {
                intent = 'REGISTER';
                intentConfidence = 0.92;
            }
            else if (textLower.includes('badal') ||
            ...
            else if (textLower.includes('apply') ||
                textLower.includes('bharna') ||
                textLower.includes('hissa')) {
                intent = 'APPLY_JOB';
                intentConfidence = 0.85;
            }
            else if (textLower.includes('manzoor') ||
            ...
            const slots = {};
            const confidence = { stt: 0.9, intent: intentConfidence, slots: {} };
            if (skill) {
                slots.skill = skill;
            ...
            VALUES (?, ?, ?, ?, 0, NULL, ?)
          `).run((0, uuid_1.v4)(), commandId, missingField, promptText, new Date().toISOString());
                }
                else {
            ...
            INSERT INTO voice_clarification_prompts (id, voice_command_id, missing_field, prompt_text, resolved, resolved_voice_command_id, created_at)
            VALUES (?, ?, ?, ?, 0, NULL, ?)
          `).run((0, uuid_1.v4)(), commandId, 'skill', 'Aap profile mein kya update karna chahte hain? Kaunsa hunar jodne chahte hain?', new Date().toISOString());
                }
                else {
        ...
        WHERE id = ?
      `).run(intent, JSON.stringify(slots), JSON.stringify(confidence), status, routedTo === 'NONE' ? null : routedTo, runId, commandId);
      ...
      `).run(runId, JSON.stringify({ commandId, transcript }), JSON.stringify({ intent, slots, confidence, status, routedTo }), JSON.stringify([commandId]), latency, new Date().toISOString());
            return {
      ...
      `).run(runId, JSON.stringify({ commandId }), JSON.stringify({ error: err.message }), Date.now() - startTime, new Date().toISOString());
            throw err;
        }
    }
    // Dynamic Skill Lookup from DB - Sourced purely from database worker profiles
    static getSkillsFromDB() {
        const list = sqlite_1.db.prepare('SELECT skills FROM worker_profiles').all();
    ...
    static getLocationsFromDB() {
        const list = sqlite_1.db.prepare('SELECT raw_text FROM job_requirements').all();
...
exports.VoiceInteractionAgent = VoiceInteractionAgent;
exports.default = VoiceInteractionAgent;
