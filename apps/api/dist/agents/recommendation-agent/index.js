...
class RecommendationAgent {
    static async run(requestReferenceId, requestReferenceType, candidateSetId, agentRunId) {
      ...
      `).run(requestReferenceId, requestReferenceType);
            for (const cand of candidates) {
                ...
                // Completed jobs
                const completedJobs = sqlite_1.db.prepare(`
          SELECT id FROM jobs WHERE worker_id = ? AND status = 'COMPLETED'
        `).all(workerId);
                const jobIds = completedJobs.map(j => j.id);
                // Ratings
                const ratings = sqlite_1.db.prepare(`
        ...
        `).all(workerId);
                ...
                // Fraud Flags
                const fraudFlags = sqlite_1.db.prepare(`
        ...
        `).all(workerId);
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run((0, uuid_1.v4)(), requestReferenceType, requestReferenceId, workerId, rank, explanation, JSON.stringify(evidence), runId, new Date().toISOString());
      ...
      `).run(runId, JSON.stringify({ requestReferenceId, requestReferenceType, candidateSetId }), JSON.stringify({ recommendationsCount: recommendations.length }), JSON.stringify([requestReferenceId, candidateSetId]), latency, new Date().toISOString());
      ...
      `).run(runId, JSON.stringify({ requestReferenceId, requestReferenceType, candidateSetId }), JSON.stringify({ error: err.message }), Date.now() - startTime, new Date().toISOString());
            throw err;
        }
    }
}
exports.RecommendationAgent = RecommendationAgent;
exports.default = RecommendationAgent;
