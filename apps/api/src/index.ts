...
async function runVerificationChecks() {
  ...
  logLines.push(`Database path: ${db.getDbPath()}`);
  ...
  if (users.length > 1) {
    ...
    const foundUser = db.prepare('SELECT * FROM users WHERE id = ?').get(secondUser.id) as any;
    ...
    const foundUserByPhone = db.prepare('SELECT * FROM users WHERE phone = ?').get(secondUser.phone) as any;
  ...
  const logPath = path.join(__dirname, '../../../database/verification_run.log');
...
try {
  ...
  const users = db.prepare('SELECT * FROM users').all();
  const debugLogPath = path.join(__dirname, '../../../database/auto_seed_debug.log');
  try {
    fs.writeFileSync(debugLogPath, JSON.stringify(users, null, 2), 'utf8');
  } catch (err: any) {}
  
  const hasNewAdmin = users.some((u: any) => u.email === 'admin@labourlink.com');
  if (!hasNewAdmin) {
  ...
  try {
    const errorLogPath = path.join(__dirname, '../../../database/startup_error.log');
    fs.writeFileSync(errorLogPath, `Failed to run database migrations/seeding:\n${e.message}\n${e.stack}\n`, 'utf8');
  } catch (err: any) {}
}
