...
export class WorkerMatchingAgent {
  static async run(requestReferenceId: string, requestReferenceType: 'JOB_REQUIREMENT' | 'SERVICE_REQUEST', agentRunId?: string): Promise<string> {
    ...
    try {
      ...
      if (requestReferenceType === 'JOB_REQUIREMENT') {
        req = db.prepare('SELECT * FROM job_requirements WHERE id = ?').get(requestReferenceId) as any;
      } else {
        req = db.prepare('SELECT * FROM service_requests WHERE id = ?').get(requestReferenceId) as any;
      }
      ...
      `).all(latMin, latMax, lngMin, lngMax) as any[];

      const matches: MatchCandidate[] = [];

      for (const row of candidatesRaw) {
        const workerSkills = JSON.parse(row.skills) as string[];
        
        // A. Skill Match Check
        const overlappingSkills = workerSkills.filter(s => reqSkills.includes(s.toLowerCase()));
        if (overlappingSkills.length === 0) continue; // No matching skill

        // B. Fine-grained Geospatial Distance Calculation
        const distRow = db.prepare('SELECT dist_km(?, ?, ?, ?) as distance').get(row.current_lat, row.current_lng, reqLat, reqLng) as { distance: number | null };
        const distance = distRow?.distance ?? 999999;
        
        if (distance > reqRadius) continue; // Outside circular radius
      ...
      for (const m of matches) {
        insertCandidate.run(
          ...
          m.worker_id,
          m.match_score,
          JSON.stringify(m.matched_fields),
      ...
      `).run(
        runId,
        JSON.stringify({ requestReferenceId, candidatesFound: candidatesRaw.length }),
        JSON.stringify({ matchCandidateSetId: candidateSetId, candidatesCount: matches.length }),
      ...
      `).run(
        runId,
        JSON.stringify({ requestReferenceId }),
...
export default WorkerMatchingAgent;
