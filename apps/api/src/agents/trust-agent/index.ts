...
export class TrustAgent {
  static async run(workerId: string, agentRunId?: string): Promise<number | null> {
    ...
    try {
      ...
      let profile = db.prepare('SELECT id, user_id, trust_score_version FROM worker_profiles WHERE id = ?').get(workerId) as any;
      let isWorker = true;
      if (!profile) {
        profile = db.prepare('SELECT id, user_id, trust_score_version FROM contractor_profiles WHERE id = ?').get(workerId) as any;
        isWorker = false;
      }
      ...
      const targetUser = db.prepare('SELECT id, phone, role FROM users WHERE id = ?').get(profile.user_id) as any;
      if (!targetUser) {
      ...
      const allEndorsements = db.prepare('SELECT * FROM endorsements').all() as any[];
      const allVerifications = db.prepare('SELECT * FROM verification_records').all() as any[];
      const allAvailability = db.prepare('SELECT * FROM availability_log').all() as any[];
      const allFraudFlags = db.prepare('SELECT * FROM fraud_flags').all() as any[];
      const allDisputes = db.prepare('SELECT * FROM disputes').all() as any[];
      const allAdminActions = db.prepare('SELECT * FROM admin_actions').all() as any[];
      const allSessions = db.prepare('SELECT * FROM auth_sessions').all() as any[];
      const allContractors = db.prepare('SELECT * FROM contractor_profiles').all() as any[];
      const allUsers = db.prepare('SELECT * FROM users').all() as any[];
      const allEngagements = db.prepare('SELECT * FROM engagements').all() as any[];
      ...
      const contributingEndorsements: any[] = [];
      const contributingVerifications: string[] = [];
      const recentChanges: any[] = [];
      const completedJobs: any[] = [];
      const lostDisputes: any[] = [];
      ...
      if (isWorker) {
        ...
        for (const end of workerEndorsements) {
          ...
          if (!contractorUser) continue;

          const isSuspended = allAdminActions.some(aa => 
            aa.action_type === 'ACCOUNT_SUSPENDED' && 
            (aa.target_id === contractor.id || aa.target_id === contractor.user_id || aa.target_id === end.endorser_id)
          );
          if (isSuspended) continue;

          const isFraudFlagged = allFraudFlags.some(ff => 
            ['OPEN', 'CONFIRMED'].includes(ff.status) && 
            (ff.subject_id === contractor.id || ff.subject_id === contractor.user_id || ff.subject_id === end.endorser_id)
          );
          if (isFraudFlagged) continue;
          ...
          if (hasSharedFp) continue;

          validEndorsements.push(end);
        }

        const distinctContractorEndorsements = new Map<string, any>();
        ...
        if (hasIdDoc) verificationFactor += 10;
        if (hasSkillCert) verificationFactor += 7;
        if (hasEmployerAttestation) verificationFactor += 3;
        ...
        const lost = allDisputes.filter(d => {
          if (d.status !== 'RESOLVED' || d.resolution !== 'worker_at_fault') return false;
        ...
        // Fraud Penalty
        const flags = allFraudFlags.filter(ff => 
          ff.subject_type === 'WORKER' && ff.subject_id === workerId && ['OPEN', 'CONFIRMED'].includes(ff.status)
        );
        ...
        const lost = allDisputes.filter(d => {
          if (d.status !== 'RESOLVED' || d.resolution !== 'contractor_at_fault') return false;
          const eng = allEngagements.find(e => e.id === d.engagement_id);
          return eng && (eng.initiator_id === profile.user_id || eng.counterparty_id === profile.user_id);
        });
        lostDisputes.push(...lost);
        ...
        const flags = allFraudFlags.filter(ff => 
          ff.subject_type === 'CONTRACTOR' && ff.subject_id === workerId && ['OPEN', 'CONFIRMED'].includes(ff.status)
        );
        activeFlags.push(...flags);
