...
export function runMigrations() {
  ...
  db.serialize(() => {
    ...
    // ALTER TABLE MIGRATIONS FOR STRUCTURED HIRING FLOW
    try { db.run(`ALTER TABLE service_requests ADD COLUMN selected_skill TEXT`); } catch(e) {}
    try { db.run(`ALTER TABLE service_requests ADD COLUMN inspection_required INTEGER DEFAULT 1`); } catch(e) {}
    try { db.run(`ALTER TABLE service_requests ADD COLUMN material_preference TEXT`); } catch(e) {}
    try { db.run(`ALTER TABLE service_requests ADD COLUMN budget_preference TEXT`); } catch(e) {}
    try { db.run(`ALTER TABLE service_requests ADD COLUMN photos TEXT`); } catch(e) {}

    try { db.run(`ALTER TABLE price_offers ADD COLUMN material_included TEXT`); } catch(e) {}
    try { db.run(`ALTER TABLE price_offers ADD COLUMN estimated_duration TEXT`); } catch(e) {}
    try { db.run(`ALTER TABLE price_offers ADD COLUMN visit_date TEXT`); } catch(e) {}

    console.log('Database migrations completed successfully.');
    try {
      const { seedCleanFreshWorkers } = require('./seed-fresh-workers');
