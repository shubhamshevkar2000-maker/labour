...
function getDbPath(): string {
  let dir = __dirname;
  for (let i = 0; i < 5; i++) {
    const checkDir = path.join(dir, 'database');
    if (fs.existsSync(checkDir)) {
      return path.join(checkDir, 'labourlink_db.json');
    }
  ...
  return path.join(__dirname, '../../../../database/labourlink_db.json');
}
...
interface DbState {
  ...
  ratings: any[];
  endorsements: any[];
  trust_score_history: any[];
  fraud_flags: any[];
  agent_run_logs: any[];
  match_candidate_sets: any[];
  match_candidates: any[];
  recommendations: any[];
  voice_commands: any[];
  voice_clarification_prompts: any[];
  auth_sessions: any[];
  notifications: any[];
  disputes: any[];
...
function dist_km(lat1: number, lon1: number, lat2: number, lon2: number): number {
  const R = 6371; // km
...
class MockStatement {
  ...
  run(...params: any[]): { changes: number; lastInsertRowid: number | string } {
    ...
    if (this.sql.match(/^DELETE FROM (\w+)/i)) {
      ...
      const tableName = match![1].toLowerCase() as keyof DbState;
      ...
      if (this.sql.includes(' WHERE ')) {
        ...
        const val = params[0];
        (data as any)[tableName] = ((data as any)[tableName] as any[]).filter(row => row[whereField] !== val);
      } else {
    ...
    if (this.sql.match(/^INSERT INTO (\w+)/i)) {
      ...
      const id = newRow.id || (columns[0] === 'id' ? params[0] : randomUUID());
      if (!newRow.id && columns[0] === 'id') {
        newRow.id = id;
      }

      ((data as any)[tableName] as any[]).push(newRow);
      saveDb();
    ...
    if (this.sql.match(/^UPDATE (\w+)/i)) {
      ...
      const matchesWhere = (row: any, bindParams: any[], startIndex: number): boolean => {
      ...
      let updatedCount = 0;
      (data as any)[tableName] = ((data as any)[tableName] as any[]).map(row => {
  ...
  all(...params: any[]): any[] {
    ...
    if (sqlLower.includes('from disputes d') && sqlLower.includes('worker_id = ?') && sqlLower.includes("status = 'resolved_contractor'")) {
      ...
      const cnt = data.disputes.filter(d => {
        if (d.status !== 'RESOLVED_CONTRACTOR') return false;
    ...
    if (sqlLower.includes('from jobs j1 join jobs j2') || sqlLower.includes('locationconflict') || sqlLower.includes('geographically separated')) {
      const results: any[] = [];
      const events: { id: string; type: 'JOB' | 'BOOKING'; worker_id: string; start: number; end: number; lat: number; lng: number }[] = [];
    ...
    if (genericMatch) {
      ...
      const tableName = genericMatch[2].trim().toLowerCase() as keyof DbState;
      ...
      if (data[tableName]) {
        let rows = [...(data[tableName] as any[])];
