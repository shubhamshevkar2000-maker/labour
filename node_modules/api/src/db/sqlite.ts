...
function getDbPath(): string {
  // Always resolve to the exact workspace root database directory
  const rootDbDir = path.resolve(__dirname, '../../../database');
  if (fs.existsSync(rootDbDir)) {
    return path.join(rootDbDir, 'labourlink_db.json');
  }
  return path.resolve(process.cwd(), 'database/labourlink_db.json');
}
...
class MockStatement {
  ...
  all(...params: any[]): any[] {
    ...
    if (sqlLower.includes('from worker_profiles') && sqlLower.includes('wp.current_lat between')) {
