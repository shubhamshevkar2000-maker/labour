...
export class WorkerMatchingAgent {
  static async run(requestReferenceId: string, requestReferenceType: 'JOB_REQUIREMENT' | 'SERVICE_REQUEST', agentRunId?: string): Promise<string> {
    ...
    try {
      ...
      const reqSkills = (JSON.parse(req.extracted_skills) as string[]).map(s => s.toLowerCase());
      ...
      // 3. Query candidates using bounding box pre-filter
      const candidatesRaw = db.prepare(`SELECT wp.id as worker_id, wp.skills, wp.current_lat, wp.current_lng, wp.trust_score, wp.verification_status, u.full_name FROM worker_profiles wp JOIN users u ON wp.user_id = u.id WHERE wp.current_lat BETWEEN ? AND ? AND wp.current_lng BETWEEN ? AND ?`).all(latMin, latMax, lngMin, lngMax) as any[];
      const matches: MatchCandidate[] = [];
      for (const row of candidatesRaw) {
        // Availability Check: Exclude workers who are dynamically busy
        const avail = AvailabilityService.getWorkerAvailability(row.worker_id);
        if (avail.status === 'BUSY') continue;

        const workerSkills = (JSON.parse(row.skills) as string[]).map(s => s.toLowerCase());
        const workerPrimarySkill = workerSkills[0] || '';
        
        // A. Strict Primary Skill Match Check (e.g. Plumber request ONLY matches Plumbers)
        const primarySkillMatches = reqSkills.some(reqSkill => 
          workerPrimarySkill.includes(reqSkill) || reqSkill.includes(workerPrimarySkill) || workerSkills.includes(reqSkill)
        );
        if (!primarySkillMatches) continue; // Exclude worker if primary skill does not match
