...
export class ApiController {
  ...
  static getWorkerProfile(req: Request, res: Response) {
    ...
    try {
      ...
      profile.calculated_availability = AvailabilityService.getWorkerAvailability(profile.id);
  ...
  static async resolveDispute(req: Request, res: Response) {
    ...
    try {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, 'TRUST_SCORE_EXCEPTION', 'JOB', ?, ?, ?)
      `).run(
        uuidv4(),
  ...
  static getCustomerServiceRequests(req: Request, res: Response) {
    ...
    try {
      const list = ServiceRequestService.getCustomerRequests(customerId as string);
      return res.status(200).json(list);
    } catch (err: any) {
      return res.status(500).json({ error: err.message });
    }
  ...
  static async proposePriceOffer(req: Request, res: Response) {
    ...
    const { offered_by, amount, note, material_included, estimated_duration, visit_date } = req.body;
    ...
    try {
      const offer = EngagementRepository.createPriceOffer({
        ...
        note: note || null,
        material_included: material_included || null,
        estimated_duration: estimated_duration || null,
        visit_date: visit_date || null
      });

      // Update engagement status to PRICE_PROPOSED
      EngagementRepository.updateStatus(engagement_id, 'PRICE_PROPOSED');
  ...
  static async respondToProposal(req: Request, res: Response) {
    ...
    try {
      ...
      if (response === 'ACCEPTED') {
        // Auto-create booking if customer profile exists and no booking created yet
        const cp = db.prepare('SELECT id FROM customer_profiles WHERE user_id = ? OR user_id = ?').get(engagement.initiator_id, engagement.counterparty_id) as any;
        if (cp) {
          const existingBooking = db.prepare('SELECT id FROM bookings WHERE service_request_id = ?').get(engagement.request_id);
          if (!existingBooking) {
            const contractorProf = db.prepare('SELECT id FROM contractor_profiles WHERE user_id = ? OR user_id = ?').get(engagement.initiator_id, engagement.counterparty_id) as any;
            BookingRepository.create({
              service_request_id: engagement.request_id,
              customer_id: cp.id,
              worker_id: wp ? wp.id : undefined,
              contractor_id: contractorProf ? contractorProf.id : undefined,
              status: 'ACCEPTED'
            });
          }
        }
