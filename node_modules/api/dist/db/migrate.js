...
function runMigrations() {
    ...
    `);
        // INDEXES
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_worker_lat_lng ON worker_profiles (home_lat, home_lng, current_lat, current_lng)`);
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_jobs_worker_status ON jobs (worker_id, status)`);
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_jobs_contractor_status ON jobs (contractor_id, status)`);
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_payments_status ON payments (status)`);
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_ratings_ratee ON ratings (ratee_id)`);
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_fraud_flags_status ON fraud_flags (status) WHERE status = 'OPEN'`);
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_voice_commands_status ON voice_commands (status)`);
        // NEW INDEXES (CUSTOMER & BOOKINGS)
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_bookings_worker_status ON bookings (worker_id, status)`);
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_bookings_customer_status ON bookings (customer_id, status)`);
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_service_requests_status ON service_requests (status)`);
        sqlite_1.db.run(`CREATE INDEX IF NOT EXISTS idx_customer_lat_lng ON customer_profiles (home_lat, home_lng)`);
